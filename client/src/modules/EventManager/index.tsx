...
function EventManager() {
  ...
  return (
    <>
      ...
      <Link to="/event-manager/events" className={`mx-3 text-decoration-none ${isActive('/event-manager/events') ? 'bold-link' : 'text-secondary'}`}>Events</Link>

      {authState?.roles?.includes('ORGANIZER')&&(
        <Link to="/event-manager/events-list" className={`mx-3 text-decoration-none ${isActive('/event-manager/events-list') ? 'bold-link' : 'text-secondary'}`}>My Events</Link>
      )}
