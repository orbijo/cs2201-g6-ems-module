  ...
  const login = (event: { preventDefault: () => void }) => {
      ...
      Axios.post("http://localhost:3001/login", {
          ...
          if(response.data.message){
              ...
              Axios.post("http://localhost:3001/role", {
                id: response.data[0].id
                }).then((res)=>{
                    if(res.data[0].title == "APPROVER"){
                        history("/event-manager/approver/events");
                    }else if(res.data[0].title == "ORGANIZER"){
                        history("/event-manager/organizer/events");
                    }else{
                        history("/event-manager/participant/events");
                    }
                })
